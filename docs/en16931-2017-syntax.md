...
| BT-82 | Payment means text | 0..1 | `cbc:PaymentMeansCode/@name` | (same) | `ram:Information` | UBL: attribute; CII: element. Corrected, see finding 9 |
...
Defects noticed while extracting this mapping. The numbering is shared with
`en16931-2026-syntax.md`, so gaps in the sequence are expected here — only findings that affect
this file are listed. Where a correction has been applied to the tables above, the affected row
says so.

### 9. BT-82 (Part 3-2, UBL) — wrong attribute casing

```
Given:    cbc:PaymentMeansCode/@Name
Expected: cbc:PaymentMeansCode/@name
```

UBL 2.1 derives `PaymentMeansCodeType` from `udt:CodeType`, whose supplementary component is the
lower case attribute `name`. There is no `@Name` attribute, so the path as given selects nothing.
The 2026 mapping document has the same row with the correct casing.
